...
v0.1.3. RavenClaw is a Bun/TypeScript coding agent. It reads and edits a workspace, runs a shell, and resumes after crash. Bring your own API keys (BYOK). No RavenClaw backend is required.
...
In the TUI: `/help`, `/resume`, `/compact`, `/cost`, `/search`, `/learn`, `/quit`. Shift+Tab cycles `default → acceptEdits → plan → default`. Escape aborts the current turn.
