...
When Bash is actually docker (backend **and** image), allowed Bash, Grep/Glob, Read/Write/Edit/ApplyPatch/ListDir/ReadSubtree, NotebookEdit bytes, and `/undo` / no-job `/rewind` restore/remove `docker run` in that container (`-v cwd:cwd -w cwd`). NotebookEdit is two execs (`cat` then `tee`+stdin). Snapshot backups stay host copies under `$RAVENCLAW_HOME/file-history/<id>/`. Omit, local, or `createTerminalBackend('docker')` without an image keeps Grep/Glob on host `rg`/walk, file tools on the host WorkspaceFs jail, NotebookEdit on host `readFileSync`/`writeFileSync` after the cwd jail, and FileHistory undo on the host. It is not a substitute for `dontAsk`. Do not treat this as docker-exec for Memory or TodoWrite.
