...
The helper lives in `packages/core/src/session/job-diff.ts` and talks to git only through `runGit` in `packages/core/src/tools/session-worktree.ts`. `runGit` is:

```ts
spawnSync('git', args, { cwd, encoding: 'utf8', timeout: GIT_TIMEOUT_MS })
```

No `env`. `cwd` is `job.worktreePath`. Spawn throw (including ENOENT) is mapped to `{ ok: false, stdout: '', stderr: '' }`, so `jobDiff` notices `'git failed'`.
...
`diff-cmd.ts` already sets `env: { ...process.env, GIT_TERMINAL_PROMPT: '0' }` but still inherits `GIT_DIR`. That sibling is **out** of this hole. `packages/core/src/tools/worktree.ts` `runGit` is the same class of hole and is also **out**.
...
1. **Locked fix: sanitize spawn env.** Production `runGit` (`session-worktree.ts`) **and** every git `spawnSync` in `serve.test.ts` copy `process.env`, keep `PATH`, set `GIT_TERMINAL_PROMPT=0`, and **delete** `GIT_DIR`, `GIT_WORK_TREE`, `GIT_INDEX_FILE`. Unique `cwd` already exists; env is what makes it win.
...
- Sanitizing `packages/core/src/tools/worktree.ts` `runGit` or `packages/cli/src/diff-cmd.ts` (same class of hole, not this flake)
