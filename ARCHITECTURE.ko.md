...
- `POST /v1/session/:id/resolve` — `{ callId, allow }` 또는 `{ callId, answer }`가 먼저 live waiter를 처리하고, 없으면 `applyAskAnswer`. optional `answer`는 `'ignored'`와 `'allow_always'`를 포함한다. `allow`/`answer`가 어긋나면 400. crash-resolve는 **pair only**. 스펙: [`2026-09-21-ignored-dismiss.md`](docs/superpowers/specs/2026-09-21-ignored-dismiss.md).
- `POST /v1/session/:id/cancel` — body에 optional `{ turnId? }`. 라이브 부모에서 stale `turnId` → **200** `{ ok: true, status: 'no_active_turn' }` (reconnect guard; walk 없음). 라이브 부모(`turnId` 생략 또는 일치) → `abort('cancel')`, join 없이 **202** `{ ok: true }`. 라이브 cancel은 자손 tree-stop이고, 이 세션 leftover-ask는 cancelled epilogue에서 I2 abort-pair한다. idle 부모 + 이 세션 leftover-ask 및/또는 자손 leftover-ask/live turn은 `whenTreeStop()` 후 **200** `{ ok: true }`. idle 부모 + 일 없음은 **200** `{ ok: true, status: 'no_active_turn' }`. `abort('interrupt')`는 이 세션만 abort-pair한다 (tree-stop 아님). 스트림은 cancel 경로에서 이 세션 I2와 tree-stop 뒤 owned unpaired 행이 남을 때만 `cancelled, ask still pending`을 낸다 (aborted에는 없음). 스펙: [`2026-09-20-leftover-ask-abort-pair.md`](docs/superpowers/specs/2026-09-20-leftover-ask-abort-pair.md).
- `POST /v1/session/:id/compact` — `compactNow()` (`liveTurn !== null`이면 큐).
- `POST /v1/session/:id/clear` — `engine.clearKeepId()`를 await하고 **200** `{ ok: true, notice: 'session cleared' }` 또는 **200** `{ ok: false, notice }`를 돌려준다 (엔진 문자열 그대로). 빈 body OK; invalid JSON은 400 `{ error: 'invalid json' }`. 없는 세션은 404 (생성하지 않음). 202/204가 아니다. tree-stop은 `/cancel`에 남긴다 (`abortCachedDescendants` / `whenTreeStop`). persist 실패와 `pending permission ask`도 200이다. 스펙: [`2026-09-20-http-post-clear.md`](docs/superpowers/specs/2026-09-20-http-post-clear.md).
...
`createSessionEngine`은 `async`이며 `Promise<SessionEngine>`을 반환한다. `job.pendingResetSha`가 있으면 construct가 `maybeFinishRewindReset`을 await한다(reset-fail는 throw하지 않는다). 세션 레코드, 메시지, 라이브 `Turn`, 태스크 레지스트리, 파일 히스토리, steering 큐를 소유한다.
...
`enqueueSteer` / `drainSteering`은 `/steer`와 `TaskSteer`가 쓰는 큐다. `bindDrainQueued`는 `/queue`의 다음 턴 텍스트를 루프에 넘긴다. `clearKeepId`는 같은 `session.id`와 같은 엔진을 유지한다. 자식 unpaired leftover-ask가 있으면 abort 전에 `pending permission ask`로 거부한다. 라이브 턴이면 `abort('cancel')` 후 idle을 기다리고 persist-first `store.clearConversation`한다. persist 실패는 memory를 바꾸지 않는다. job/worktree/락/MCP/자식은 유지. serve `POST /v1/session/:id/clear`는 `engine.clearKeepId()`를 await하고 **200** `{ ok, notice }`를 돌려준다. persist 실패와 `pending permission ask`도 200이다. 없는 세션은 404 (생성하지 않음). tree-stop은 `/cancel`에 남긴다. 이 경로에서 `openNewSession` / `close` / `mcpCloser`는 호출하지 않는다. `rewindLast`는 라이브 턴·running agent·unpaired pending ask가 없을 때만 동작하며 `{ ok, notice, droppedText? }`를 반환한다. `job.pendingResetSha`가 있으면 먼저 `maybeFinishRewindReset`을 await한다. **job 세션** (`session.job`)은 `rewindToCheckpoint`: 드롭된 id에 대해 compact `rewind` persist, 대상 sha를 `job.pendingResetSha`에 upsert, 그 다음 job worktree에서 이전 assistant 체크포인트 sha(또는 `baseCommitSha`)로 `git reset --hard`, 해당 todo 스냅샷을 `session.todos`에 복원, `pendingResetSha` clear, `originalCwd ?? cwd` 아래 프로젝트 `.ravenclaw/todo.json` 재투영. persist 실패 시 HEAD와 마지막 user는 그대로. flag 기록 후 reset 실패 시 `pendingResetSha`와 `jobError`를 유지하고, construct / `submitMessage` / `rewindLast` / host `/diff`가 끝낸다. compact+flag는 한 SQLite write(`recordCompactAndUpsertSession`)다. `createSessionEngine`은 async recover 지점이다. GET snapshot은 엔진을 attach하지 않는다. projection 실패는 notice suffix (`ok`는 true). **job 없는 세션**은 `rewindLastTurn`(file-history undo + 마지막 user 턴 drop + 남은 assistant `todoSnapshot`으로 `session.todos` 복원 후 `session.cwd`의 `todo.json` 재투영)이다. checkpoint가 없는 예전 transcript는 todos를 건드리지 않는다. `/undo`는 파일만. serve `POST …/edit`와 TUI `/retry`는 rewind 후 `submitMessage`(또는 composer 복원)를 합성한다. `setFollowup` / `clearFollowup` / `getFollowup`은 `session.followup` one-slot(schema v10)이다. `writeFollowup`은 memory assign 전에 `upsertSession`한다. 호스트는 실제 턴 종료 후 `runFollowupAfterSubmit`을 호출한다 (persist된 `lastEnd`만; owned leftover-ask는 skip). `abort(kind?)`는 background review를 취소하고 라이브 턴을 abort한다. serve cancel은 `'cancel'`을 넘긴다. `abort('cancel')`은 자손 tree-stop이며, 이 세션 leftover-ask는 live 여부와 상관없이 abort-pair한다. 라이브 cancel의 this-session I2는 cancelled epilogue가 소유하고, idle cancel은 leftover flight를 시작한다. leftover/tree-stop persist는 `whenTreeStop()`을 await할 때 시작한다. `whenTreeStop()`은 `{ descendantWork, thisSessionWork }`를 반환한다. `abort('interrupt')`는 이 세션만 abort-pair한다 (tree-stop 아님; 부모 interrupt는 parked 자식 leftover-ask를 남긴다). idle this-session I2는 `lastEnd`를 만들지 않는다. 스트림은 cancel I2 + tree-stop 뒤 owned unpaired 행이 남을 때만 `cancelled, ask still pending`을 낸다 (aborted에는 없음). `liveTurnId()`가 serve `turnId` 가드를 받친다. `close`는 `SessionEnd` 후 락을 놓는다.
...
| 툴 | 역할 |
|---|---|
| `Read` `Grep` `Glob` `ListDir` `ReadSubtree` | 읽기. `.ipynb`는 셀로 보여 준다. Grep/Glob와 Read/ListDir/ReadSubtree는 Bash와 같은 `TerminalBackend`를 공유한다. docker(+image)면 Grep/Glob는 컨테이너에서 `backend.exec` 한 번(POSIX walker); Read/ListDir/ReadSubtree는 호스트 jail 후 WorkspaceFs 메서드당 exec 한 번. 실패는 `Grep failed:` / `Glob failed:` / `Read failed:` / `ListDir failed:` / `ReadSubtree failed:`(호스트 fallback 없음). 턴 abort는 `AbortError` / `ABORTED_TEXT`. omit/local/이미지 없는 docker는 호스트 `rg`/walk / 호스트 WorkspaceFs jail |
| `Edit` `Write` `ApplyPatch` | 쓰기. Bash와 같은 `TerminalBackend`. docker(+image)면 호스트 jail 후 WorkspaceFs 메서드당 exec 한 번; 실패는 `Edit failed:` / `Write failed:` / `ApplyPatch failed:`. 턴 abort는 `AbortError` / `ABORTED_TEXT`. omit/local/이미지 없는 docker는 호스트 WorkspaceFs jail. `Edit`는 CRLF/indent를 견딘다. 트리 안 `ApplyPatch`는 `acceptEdits`/`dontAsk`에서 승격. FileHistory snapshot은 호스트 |
...
SQLite WAL, `$RAVENCLAW_HOME/state.db`. `PRAGMA journal_mode = WAL`, `busy_timeout = 5000`, `foreign_keys = ON` (`packages/core/src/session/sqlite-store.ts`). Schema version **11**:
...
이 프로세스는 사용자와 같은 OS 유저다. 네트워크 샌드박스는 없다. Docker terminal backend(kind+image)는 허용된 Bash, Grep/Glob, Read/Write/Edit/ApplyPatch/ListDir/ReadSubtree가 어디서 도는지 바꿀 뿐, 권한 결정을 바꾸지 않는다. omit/local/이미지 없는 docker는 Grep/Glob가 호스트 `rg`/walk, 파일 툴은 호스트 WorkspaceFs jail. NotebookEdit·Memory·file-history writer는 호스트다. `dontAsk` 대체가 아니다.
...
- **terminal backend** — `local` | `docker`. Bash, Grep/Glob, Read/Write/Edit/ApplyPatch/ListDir/ReadSubtree가 같은 객체를 쓴다.
