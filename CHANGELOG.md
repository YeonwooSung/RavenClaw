...
- Job-host state on `feat/job-host-state` (not marked shipped until it lands on `main`): schema **10** (`last_end_json`, `job_error`, `followup_text`); GET snapshot fields `title`, `jobAutoCommit`, `lastEnd`, `jobError`, `queued`; `POST/DELETE …/followup`; `POST …/edit`; `GET …/diff`; TUI `/follow`, `/retry`, and job-range `/diff`. Spec: [docs/superpowers/specs/2026-09-17-job-host-state-roadmap.md](docs/superpowers/specs/2026-09-17-job-host-state-roadmap.md). Plan: [docs/superpowers/plans/2026-09-17-job-host-state-implementation.md](docs/superpowers/plans/2026-09-17-job-host-state-implementation.md).
