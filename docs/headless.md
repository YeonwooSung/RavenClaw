...
- `POST /v1/session/:id/cancel` — optional `{ turnId }` from `round_start`. Calls `engine.abort('cancel')` when the live turn matches (or any live turn if `turnId` is omitted). Stale `turnId` while the parent is live → **200** `{ ok: true, status: 'no_active_turn' }` (reconnect guard). Live parent cancel is tree-stop of descendants. Idle parent with descendant leftover-asks or in-process descendant live turns is **200** `{ ok: true }` after `whenTreeStop()`. Idle parent with no descendant work is **200** `{ ok: true, status: 'no_active_turn' }` and does **not** drop this session’s parked asks. `abort('interrupt')` is not tree-stop. The stream emits `cancelled, ask still pending` only if an owned unpaired row remains. Cancel is not failure — the session accepts the next `submit`. Spec: [`superpowers/specs/2026-09-18-parent-tree-stop.md`](superpowers/specs/2026-09-18-parent-tree-stop.md).
