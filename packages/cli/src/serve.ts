...
export async function runServe(opts: { flags: ConfigFlags }): Promise<number> {
  ...
  const runtimeFor = async (sessionKey: string | undefined, webhook: boolean) => {
    ...
    const flags = {
      ...opts.flags,
      dontAsk: true as const,
      ...(webhook ? { allowedTools: safeWebhookToolNames() } : {}),
    }
    const boot = await bootCli({ flags, createSession: false, surface: 'headless' })
    ...
    async fetch(req) {
      ...
      if (req.method === 'POST' && url.pathname === '/v1/turn') {
        ...
        try {
          ...
          const result = await runExec({ prompt: parsed.text, engine: runtime.engine })
        ...
        void (async () => {
          try {
            const flags = {
              ...opts.flags,
              dontAsk: true as const,
              allowedTools: safeWebhookToolNames(),
            }
            const boot = await bootCli({ flags, createSession: false, surface: 'headless' })
            const opened = await openNewSession(boot)
