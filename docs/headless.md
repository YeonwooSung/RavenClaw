...
When Bash is actually docker (backend **and** image), allowed Bash, Grep/Glob, Read/Write/Edit/ApplyPatch/ListDir/ReadSubtree, NotebookEdit bytes, in-tree Memory (`.ravenclaw/MEMORY.md` / `USER.md` inside `turn.cwd`), and `/undo` / no-job `/rewind` restore/remove `docker run` in that container (`-v cwd:cwd -w cwd`). NotebookEdit is two execs (`cat` then `tee`+stdin). Memory uses WorkspaceFs (`stat`/`readFile`, `mkdir`, `tee`+stdin). Job isolation (`projectCwd` outside `cwd`) on docker is `Memory failed: outside workspace` with no exec and no host write. Snapshot backups stay host copies under `$RAVENCLAW_HOME/file-history/<id>/`. Omit, local, or `createTerminalBackend('docker')` without an image keeps Grep/Glob on host `rg`/walk, file tools on the host WorkspaceFs jail, NotebookEdit on host `readFileSync`/`writeFileSync` after the cwd jail, Memory on host `node:fs` (including the `projectCwd` sidecar), and FileHistory undo on the host. It is not a substitute for `dontAsk`. Do **not** treat this as docker-exec for TodoWrite or Skill.
...
- `POST /v1/session/:id/submit` — `{ text }` → `submitMessage` with `turnPolicy: queue` (202 accepted). Parked leftover-ask + submit text persist-then-drops `IGNORED_TEXT` then continues (quiet; not a `pending permission ask` no-op). Live turn still blocks. Missing sessions are created with `default` permission mode (not `dontAsk`). Spec: [`superpowers/specs/2026-09-21-dismiss-on-message.md`](superpowers/specs/2026-09-21-dismiss-on-message.md).
- `POST /v1/session/:id/cancel` — optional `{ turnId }` from `round_start`. Stale `turnId` while the parent is live → **200** `{ ok: true, status: 'no_active_turn' }` (reconnect guard; no walk). Live parent (`turnId` omitted or matching) → `abort('cancel')`, **202** `{ ok: true }` without joining `whenTreeStop`. Live cancel is tree-stop of descendants; this session’s leftover-asks abort-pair in the cancelled epilogue. Idle parent with this-session leftover-asks and/or descendant leftover-asks / in-process descendant live turns is **200** `{ ok: true }` after `whenTreeStop()`. Idle parent with no work is **200** `{ ok: true, status: 'no_active_turn' }`. `abort('interrupt')` abort-pairs this session only (not tree-stop). The stream emits `cancelled, ask still pending` only if an owned unpaired row remains after cancel I2 + tree-stop (not on aborted). Cancel is not failure — the session accepts the next `submit`. Spec: [`superpowers/specs/2026-09-20-leftover-ask-abort-pair.md`](superpowers/specs/2026-09-20-leftover-ask-abort-pair.md).
...
- `POST /v1/session/:id/resolve` — `{ callId, allow: boolean }` or `{ callId, answer }` (`answer` may be `'allow'|'deny'|'allow_always'|'ignored'`) → live waiter or `applyAskAnswer`; disagreeing fields 400; 200 `{ status }` or 404 if unmatched. Crash-resolve `'ignored'` pairs `IGNORED_TEXT` only (no model turn). Spec: [`superpowers/specs/2026-09-21-ignored-dismiss.md`](superpowers/specs/2026-09-21-ignored-dismiss.md). Dismiss-on-message of parked leftover-asks is a **new user message** through `POST …/submit`, not resolve. Spec: [`superpowers/specs/2026-09-21-dismiss-on-message.md`](superpowers/specs/2026-09-21-dismiss-on-message.md).
