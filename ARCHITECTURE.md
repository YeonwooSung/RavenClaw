...
- `POST /v1/session/:id/cancel` — optional body `{ turnId? }`. Calls `engine.abort('cancel')` when the live turn matches (or any live turn if `turnId` omitted). Stale / no live turn → **200** `{ ok: true, status: 'no_active_turn' }` and does not touch leftover-asks. Live `abort('cancel')` abort-pairs **this** session’s leftover-asks (drop-only when already transcript-paired; unpaired persist one `ABORTED_TEXT` then drop). `abort('interrupt')` and child leftover-asks are unchanged. Stream emits `cancelled, ask still pending` only if a row remains.
...
- **`abort(kind?)`** cancels background review and `abortTurn(liveTurn.abort)`. Serve cancel passes `'cancel'` (vs `'interrupt'` for steer). Live `abort('cancel')` abort-pairs **this** session’s leftover-asks (drop-only when already paired; unpaired persist `ABORTED_TEXT` then drop). No live turn, `abort('interrupt')`, and child leftover-asks are unchanged. Stream emits `cancelled, ask still pending` only if a row remains. Optional `liveTurnId()` backs the serve `turnId` guard.
