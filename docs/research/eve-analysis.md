...
RavenClaw leftover-ask now has an `'ignored'` operator result ([`2026-09-21-ignored-dismiss.md`](../superpowers/specs/2026-09-21-ignored-dismiss.md)). leftover-ask now has dismiss-on-message via `submitMessage` ([`2026-09-21-dismiss-on-message.md`](../superpowers/specs/2026-09-21-dismiss-on-message.md)). Approvals still do not dismiss-on-message.
...
RavenClaw already has `terminal.backend: docker` for **Bash only**. Read/Write/Edit still hit the host. Steal: **when a sandbox backend is on, file tools go through the same port**. Secrets stay out of the container env (we already allowlist docker env). Closer: Grep/Glob now share Bash’s docker `TerminalBackend` when kind+image construct docker (`docs/superpowers/specs/2026-09-21-grep-glob-docker-exec.md`). Read/Write/Edit/ApplyPatch/ListDir/ReadSubtree share that same port (`docs/superpowers/specs/2026-09-21-workspacefs-docker.md`). NotebookEdit bytes share that same port when kind+image (`docs/superpowers/specs/2026-09-21-notebookedit-docker.md`). Memory in-tree bytes share Bash’s docker port when kind+image (`docs/superpowers/specs/2026-09-22-memory-docker.md`); job isolation (`projectCwd` outside cwd) fail-closes. FileHistory undo restore/remove shares Bash’s docker port when kind+image and injected (`docs/superpowers/specs/2026-09-22-file-history-docker.md`). Snapshot backups stay host.
...
That eve-inspired horizon is implemented. Session-as-job is implemented (`docs/superpowers/specs/2026-09-16-session-as-job-roadmap.md`). Job-host state is implemented (`docs/superpowers/specs/2026-09-17-job-host-state-roadmap.md` at `6e56764`). Rewind persist-before-reset is implemented (`docs/superpowers/specs/2026-09-18-rewind-persist-and-todo-projection.md` at `048deff`). Cancel abort-pair / reset-on-resume / follow-up persist is implemented (`docs/superpowers/specs/2026-09-18-cancel-reset-followup.md` at `5eefdde`). No-job todo revert is implemented (`docs/superpowers/specs/2026-09-18-no-job-todo-revert.md` at `be5a4a7`). Stream version / `continuationToken` is implemented (`docs/superpowers/specs/2026-09-18-stream-version-token.md` at `c9c4871`). Keep-id `/clear` is implemented (`docs/superpowers/specs/2026-09-18-keep-id-clear.md` at `edeb611`). Parent tree-stop is implemented (`docs/superpowers/specs/2026-09-18-parent-tree-stop.md` at `9901d0e`). Leftover-ask abort-pair completeness + cancel 202/200 is implemented (`docs/superpowers/specs/2026-09-20-leftover-ask-abort-pair.md` at `067bfba`). Grep/Glob docker-exec is implemented (`docs/superpowers/specs/2026-09-21-grep-glob-docker-exec.md` at `a52eab1`). WorkspaceFs docker I/O is implemented (`docs/superpowers/specs/2026-09-21-workspacefs-docker.md`). NotebookEdit docker is implemented (`docs/superpowers/specs/2026-09-21-notebookedit-docker.md`). Ignored dismiss-and-continue is implemented (`docs/superpowers/specs/2026-09-21-ignored-dismiss.md` at `22a55c1`). leftover-ask now has dismiss-on-message via `submitMessage` (`docs/superpowers/specs/2026-09-21-dismiss-on-message.md`). Approvals still do not dismiss-on-message. Bounded LSP depth is implemented (`docs/superpowers/specs/2026-09-21-lsp-depth.md` at `6c797d1`). Slack / Discord / ACP skip UI now emits `'ignored'` ([`2026-09-22-hitl-skip-hosts.md`](../superpowers/specs/2026-09-22-hitl-skip-hosts.md)). Timeout / editor `cancelled` stay. CI unit coverage of every workspace package is implemented (`docs/superpowers/specs/2026-09-22-ci-unit-packages.md`).
