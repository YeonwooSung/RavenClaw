...
RavenClaw 대응: `once()` ≈ `allow_always`, `always()` ≈ leftover-ask, `ask_question` ≈ `AskUser`. leftover-ask는 이제 `'ignored'` 결과가 있다 ([`2026-09-21-ignored-dismiss.md`](../superpowers/specs/2026-09-21-ignored-dismiss.md)). leftover-ask는 이제 `submitMessage`로 dismiss-on-message한다 ([`2026-09-21-dismiss-on-message.md`](../superpowers/specs/2026-09-21-dismiss-on-message.md)). 승인은 메시지로 dismiss-on-message하지 않는다. **이름 있는 정책 + durable pending 배치**를 가져온다. 기본 자동 실행은 가져오지 않는다.
...
그 eve 호라이즌은 구현됐다. 세션-을-잡은 구현됐다 (`docs/superpowers/specs/2026-09-16-session-as-job-roadmap.md`). Job-host state는 구현됐다 (`docs/superpowers/specs/2026-09-17-job-host-state-roadmap.md`, `6e56764`). Rewind persist-before-reset는 구현됐다 (`docs/superpowers/specs/2026-09-18-rewind-persist-and-todo-projection.md`, `048deff`). Cancel abort-pair / reset-on-resume / follow-up persist는 구현됐다 (`docs/superpowers/specs/2026-09-18-cancel-reset-followup.md`, `5eefdde`). No-job todo revert는 구현됐다 (`docs/superpowers/specs/2026-09-18-no-job-todo-revert.md`, `be5a4a7`). Stream version / `continuationToken`은 구현됐다 (`docs/superpowers/specs/2026-09-18-stream-version-token.md`, `c9c4871`). Keep-id `/clear`는 구현됐다 (`docs/superpowers/specs/2026-09-18-keep-id-clear.md`, `edeb611`). Parent tree-stop은 구현됐다 (`docs/superpowers/specs/2026-09-18-parent-tree-stop.md`, `9901d0e`). Leftover-ask abort-pair completeness + cancel 202/200은 구현됐다 (`docs/superpowers/specs/2026-09-20-leftover-ask-abort-pair.md`, `067bfba`). Grep/Glob docker-exec는 구현됐다 (`docs/superpowers/specs/2026-09-21-grep-glob-docker-exec.md`, `a52eab1`). WorkspaceFs docker I/O는 구현됐다 (`docs/superpowers/specs/2026-09-21-workspacefs-docker.md`). NotebookEdit docker는 구현됐다 (`docs/superpowers/specs/2026-09-21-notebookedit-docker.md`). Ignored dismiss-and-continue는 구현됐다 (`docs/superpowers/specs/2026-09-21-ignored-dismiss.md`, `22a55c1`). leftover-ask는 이제 `submitMessage`로 dismiss-on-message한다 (`docs/superpowers/specs/2026-09-21-dismiss-on-message.md`). 승인은 메시지로 dismiss-on-message하지 않는다. Bounded LSP depth는 구현됐다 (`docs/superpowers/specs/2026-09-21-lsp-depth.md`, `6c797d1`). Slack / Discord / ACP skip UI는 이제 `'ignored'`를 낸다 ([`2026-09-22-hitl-skip-hosts.md`](../superpowers/specs/2026-09-22-hitl-skip-hosts.md)). Timeout / 에디터 `cancelled`는 그대로다. 모든 워크스페이스 패키지의 CI 유닛 커버리지는 구현됐다 (`docs/superpowers/specs/2026-09-22-ci-unit-packages.md`).
