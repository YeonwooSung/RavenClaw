...
| Escape | If the diff panel is open and no AskUser is pending: close diff. Else reject a pending AskUser as `AbortError`, call `engine.abort()`, and run the second-abort gate (see `/stop`). If a resume picker is open, close it. If idle (not busy, no AskUser): clear the draft. |
