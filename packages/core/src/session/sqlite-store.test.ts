...
import { createSqliteStore, searchSessionStore, sqliteStoreDatabase } from './sqlite-store'
...
describe('createSqliteStore', () => {
  ...
  test('clearConversation wipes this session and keeps job, child, and rules', async () => {
    ...
    const boundary = db!
      .query(`SELECT generation, summary FROM compact_boundaries WHERE session_id = ?`)
      .get('s1') as { generation: number; summary: string }
    expect(boundary).toEqual({ generation: 1, summary: 'clear' })
  ...
  test('clearConversation with no ids skips the compact boundary', async () => {
    const store = openStore()
    ...
    const db = sqliteStoreDatabase(store)
    expect(
      db!.query(`SELECT 1 AS ok FROM compact_boundaries WHERE session_id = ?`).get('s1'),
    ).toBeNull()
  })
  ...
  test('clearConversation throw leaves transcript, asks, and stream', async () => {
    ...
    await store.recordCompact('s1', 1, 'prior', ['u1'])
    await store.persistUser('s1', {
      id: 'u2',
      role: 'user',
    ...
    const loaded = await store.loadSession('s1')
    expect(loaded.messages.map((msg) => msg.id)).toEqual(['u2'])
